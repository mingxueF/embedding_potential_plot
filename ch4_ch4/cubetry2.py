...
dmA = np.load("dm0_ref"+basis_str+".npy")
dmA_iso = np.load('dmA_iso.npy')
dmB = np.load("dm1_ref"+basis_str+".npy") 
# =============================================================================
# define a grid for plotting
# =============================================================================
cc = cubegen.Cube(system,nx=40, ny=40, nz=40,margin=5)
points = cc.get_coords()
dmA_mp2 = np.load("dm0_mp2"+basis_str+".npy")
dmA_diff = dmA_mp2 - dmA_iso
rho0_diff,rho1,rho_both = functions.get_density(wrap.mol0,wrap.mol1,dmA_diff,dmB,points)
rho0_diff = rho0_diff.reshape(cc.nx,cc.ny,cc.nz)
cc.write(rho0_diff,'rho_diff_iso'+basis_str+'.cube','density difference between refHF and mp2')
def emb_potential(points,xc_code,t_code,elec=True,non_k = True,non_xc = True):
    ...
    if elec == True:        
        ...
        if non_k == True and non_xc == True:
            ...
            cc.write(vemb_tot,'emb'+ basis_str+'.cube','Molecular embedding potential in real space')
