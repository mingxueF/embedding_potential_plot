...
dmA_mp2 = np.load("dm0_mp2"+basis+".npy")
dmA_ref = np.load("dm0_ref"+basis+".npy")
dmB = np.load("dm1_ref"+ basis +".npy")
dmA_iso = np.load(os.path.join(root,'Dm/'+basis+'/dmA_iso.npy'))
# =============================================================================
# define a grid for plotting
# =============================================================================
#cc = cubegen.Cube(system,nx=0, ny=0, nz=20,margin=5)
#points = cc.get_coords()
one_d = np.arange(-4.,4.,0.1)
...
np.save('rho_diff'+basis,rho0_diff)
#-----------------------------------------------------------------------------
def emb_potential(points,xc_code,t_code,elec=True,non_k = True,non_xc = True):
    ...
    if elec == True:        
        ...
        if non_k == True and non_xc == True:
            ...
            np.save(basis,vemb_tot)
            plt.plot(one_d,vemb_tot,label='emb_potential')
            plt.show()
            plt.savefig(basis,dpi = 800)
